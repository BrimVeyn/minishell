...
/*   Updated: 2024/01/26 11:21:27 by bvan-pae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (ms_setint(&x[J], ZERO), split[x[I]])
	{
		...
		while (split[x[I]][x[J]])
        {
			if (split[x[I]][x[J]] != '\'' && split[x[I]][x[J]] != '\"')
				ms_starlab(&strl, ms_starlnew(r_env(ms_extract(split[x[I]], &x[J], ZERO), denv), 0));
			else if (split[x[I]][x[J]] == '\"')
				ms_starlab(&strl, ms_starlnew(r_env(ms_extract(split[x[I]], &x[J], '\"'), denv), 0));
			else if (split[x[I]][x[J]] == '\'')
