...
/*   Updated: 2024/02/01 10:37:06 by nbardavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int handle_append_redirect(char *token, t_pipe *d_pipe)
{
...
static int handle_output_redirect(char *token, t_pipe *d_pipe)
{
...
static int handle_input_redirect(char *token, t_pipe *d_pipe) 
{
...
int cmd_redi(t_tok *d_token, t_pipe *d_pipe, int *i, int j) 
{
    ...
    while (d_token->tokens[*i][j]) 
	{
        if (!ft_strcmp(d_token->tokens[*i][j], ">>"))
            d_pipe->failure = handle_append_redirect(d_token->tokens[*i][++j], d_pipe);
        else if (!ft_strcmp(d_token->tokens[*i][j], ">"))
            d_pipe->failure = handle_output_redirect(d_token->tokens[*i][++j], d_pipe);
        else if (!ft_strcmp(d_token->tokens[*i][j], "<"))
            d_pipe->failure = handle_input_redirect(d_token->tokens[*i][++j], d_pipe);
        else
